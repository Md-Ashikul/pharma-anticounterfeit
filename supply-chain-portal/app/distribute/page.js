...
export default function DistributePage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res    = await supplyAPI.distribute(client, {
        drugId:   drugId.trim(),
    ...
    <RoleGate roles={["Distributor"]}>
      <div className="page">
        ...
        <p className="text-muted" style={{ marginBottom: "1.5rem" }}>
          Scan the Public QR or enter the Drug ID to record receiving this strip.
        </p>
        <div className="card">
          ...
          <input
            className="input"
            placeholder="e.g. Chittagong Warehouse"
            value={location}
          ...
          {result && (
            <div className="alert alert-success">
              ✅ Distribution recorded for <strong>{result.drugId}</strong>.
          ...
          >
            {loading ? <><span className="spinner" /> Recording...</> : "Record Distribution"}
          </button>
        </div>
        {history && (
          <DrugTimeline
